    ...
    , useVsync{false}
{
    ...
    ETNA_ASSERTF(pixels != nullptr, "failed to load texture image!");

    texture_ = context.createImage(etna::Image::CreateInfo{
    ...
    std::unique_ptr<etna::OneShotCmdMgr> oneShotCmdMgr = context.createOneShotCmdMgr();
    ...
    for (size_t i = 0; i < NumFramesInFlight; i++) {
        auto& buff = frameIter.next();
        buff = etna::get_context().createBuffer(etna::Buffer::CreateInfo{
...
void App::drawFrame()
{
    ...
    if (nextSwapchainImage)
    {
        ...
        auto& buffer = frameIter.next();
        std::memcpy(buffer.data(), &params, sizeof(params));
                ...
                auto set = etna::create_descriptor_set(
                    ...
                    currentCmdBuf,
                    {
                        ...
                        etna::Binding{1, texture_.genBinding(sampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal)},
                         etna::Binding{2, buffer.genBinding()},
                    }
                );
