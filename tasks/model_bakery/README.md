...
1. Убедитись, что устанговлен go 1.23
```go version```
Если это не так, то на Windows http://go.dev/
Для linux ```sudo snap install go --classic```

2. Запустите скрипт в корне репозитория, он соберет проект

3. запуститие бинарник ```baker <file>```
